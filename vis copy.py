...
def load_data(data_dir):
    ...
    synaptic_short_file = os.path.join(data_dir, 'synaptic_short_data_clean.csv')
    ...
    synaptic_long_file = os.path.join(data_dir, 'synaptic_long_data_clean.csv')
...
def plot_summary(data, output_dir):
    """Plot overall system metrics summary with log scale and percent change"""
    ...
    # --- Plot 1: Baseline vs Final (log scale) ---
    fig1, ax1 = plt.subplots(figsize=(12, 6))
    ax1.bar(x - width/2, baseline, width, label='Baseline')
    ax1.bar(x + width/2, final, width, label='Final')

    ax1.set_yscale('log')
    ax1.set_ylabel('Value (log scale)')
    ax1.set_title('System Metrics: Baseline vs Final (Log Scale)')
    ax1.set_xticks(x)
    ...
    # --- Plot 2: Percent Change Only ---
    fig2, ax2 = plt.subplots(figsize=(10, 6))
    bars = ax2.bar(x, percent_change, width=0.5, color='skyblue')

    ax2.axhline(100, color='gray', linestyle='--', linewidth=1)
    ax2.set_ylabel('Percent of Baseline (%)')
    ax2.set_title('System Metrics: Change from Baseline')
    ax2.set_xticks(x)
