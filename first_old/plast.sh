...
# Sysbench requires careful execution - run quieter
run_sysbench() {
...
cleanup() {
  ...
  rm -f hammer hammer.c spec_havoc spec_havoc.o spec_havoc.S "$TEST_DATA_FILE" turbostat.log sensors.log mbw.log
...
# Compilation code omitted for brevity (same as previous version)
gcc -O2 -march=native -o hammer hammer.c || { echo "!!! Hammer compilation failed!"; exit 1; }
as spec_havoc.S -o spec_havoc.o || { echo "!!! Spec Havoc assembly failed!"; exit 1; }
ld spec_havoc.o -o spec_havoc || { echo "!!! Spec Havoc linking failed!"; exit 1; }
...
if [ "$MODE" == "ALL" ] || [ "$MODE" == "SYNAPSE_SHORT" ]; then
    ...
    for cycle in $(seq 1 $SYN_ST_CYCLES); do
        ...
        sleep 2
        current_bench_score=$(run_sysbench) # Measure before potentiation
        ...
        sleep 2
        current_bench_score=$(run_sysbench) # Final score after forgetting
        ...
        if [[ "$current_bench_score" =~ ^[0-9]+(\.[0-9]+)?$ && "$baseline_bench_score" =~ ^[0-9]+(\.[0-9]+)?$ ]]; then
             ...
             log_message "  Recovery comparison skipped (invalid scores)."
         fi
...
if [ "$MODE" == "ALL" ] || [ "$MODE" == "SYNAPSE_LONG" ]; then
    ...
    for cycle in $(seq 1 $SYN_LT_CYCLES); do
        ...
        if dmesg | tail -n 20 | grep -q -iE 'MCE|uncorrected|critical|panic'; then
             ...
             ltp_errors=true
             break # Stop cycling on critical error
        fi
        ...
        if (( cycle % 10 == 0 )); then
             ...
             if [ "$md5_orig" != "$md5_now" ]; then
                 ...
                 ltp_corruption=true
                 break # Stop cycling on corruption
             fi
...
final_bench_score=$(run_sysbench) # One last check overall
...
log_message "Final Sysbench Score: $final_bench_score events/sec (Baseline: $baseline_bench_score)"
