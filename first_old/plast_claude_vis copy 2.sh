...
# Improved sysbench function that properly captures only the numeric score
run_sysbench() {
    ...
    if [ "$ltp_errors" = false ] && [ "$ltp_corruption" = false ]; then
        ...
        while true; do
            ...
            if (( elapsed_retention % 10 == 0 )); then
                ...
                retention_points+=($elapsed_retention)
                retention_temps+=($temp_current)
                retention_scores+=($bench_score_current)
